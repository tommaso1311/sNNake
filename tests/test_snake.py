...
def test_init_snake(fixture_snake):
	...
	assert fixture_snake._direction in ['R', 'D', 'L', 'U']
...
def test_has_not_eaten_food(fixture_snake, fixture_food):
	"""
	...
	assert fixture_snake.has_not_eaten_food(fixture_food)

	fixture_food.position = np.array([1, 1])
	fitness_before = fixture_snake.fitness
	length_before = fixture_snake.length

	assert not fixture_snake.has_not_eaten_food(fixture_food)
...
def test_get_status(fixture_snake, fixture_food, fixture_size):
	...
	for i in range(len(fixture_snake._directions)):
		...
		fixture_snake._direction = fixture_snake._directions[i]
		fixture_snake._get_status(fixture_size, fixture_food)
